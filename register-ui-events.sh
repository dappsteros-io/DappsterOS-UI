...
runtime_file="/var/run/dappsteros/message-bus.url"
ui_message_bus_file="/var/lib/dappsteros/ui-message-bus.json"
if [ -f "$runtime_file" ] && [ -f "$ui_message_bus_file" ]
then
    MESSAGE_BUS_URL=$(cat /var/run/dappsteros/message-bus.url)
