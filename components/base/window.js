...
export class Window extends Component {
    ...
    setDefaultWindowDimenstion = () => {
        ...
        if (window.innerWidth < 600) {
            this.setState({ height: 400, width: 400 }, this.resizeBoundries);
        }
        else {
            this.setState({ height: 500, width: 500 }, this.resizeBoundries);
        }
    }
    ...
    checkOverlap = (e) => {
        ...
        if (e) {
            this.startX = Math.min(Math.max(0, this.startX + e.movementX), window.innerWidth)
            this.startY = Math.min(Math.max(0, this.startY + e.movementY), window.innerHeight)
            this.setState({ x: this.startX, y: this.startY })
        }
    }
    ...
    maximizeWindow = () => {
        if (this.state.maximized) {
            ...
            this.setState({ maximized: true, height: window.innerHeight, width: window.innerWidth });
            this.props.hideSideBar(this.id, true);
        }
        this.checkOverlap();
    }
    ...
    onResize = (event, { node, size, handle }) => {
        this.setState({ width: size.width, height: size.height, maximized: false })

    };
    onResizeStop = (event, { node, size, handle }) => {
        if (["e", "s", "se"].indexOf(handle) == -1) {
            this.startX = this.startX + event.movementX
            this.startY = this.startY + event.movementY
        }
        this.setState({ width: size.width, height: size.height, x: this.startX, y: this.startY, maximized: false })
    ...
    render() {
        return (
            ...
            >
                <Resizable className='resizable-panel' width={this.state.width} height={this.state.height} onResize={this.onResize} onResizeStop={this.onResizeStop} resizeHandles={['s', /* 'w', */ 'e', /* 'n', */ 'sw', /* 'nw', */ 'se',/*  'ne' */]} onResizeStart={() => { }} children={<></>}>
                    <div style={{ width: `${this.state.width}px`, height: `${this.state.height}px` }}
                        className={this.state.cursorType + " !absolute box " + (this.state.closed ? " closed-window " : "") + (this.state.maximized ? " duration-300 rounded-none" : " rounded-lg rounded-b-none") + (this.props.minimized ? " opacity-0 invisible duration-200 " : "") + (this.props.isFocused ? " z-30 " : " z-20 notFocused") + " opened-window overflow-hidden min-w-1/4 min-h-1/4 main-window absolute window-shadow border-black border-opacity-40 border border-t-0 flex flex-col"}
                        id={this.id}
                    >
                        ...
                        {(this.id === "settings"
                            ? <Settings dataContext={this.props.id} changeBackgroundImage={this.props.changeBackgroundImage} currBgImgName={this.props.bg_image_name} size={{ width: this.state.width, height: this.state.height }} />
                            : <WindowMainScreen dataContext={this.props.id} screen={this.props.screen} title={this.props.title}
                                addFolder={this.props.id === "terminal" ? this.props.addFolder : null}
                                openApp={this.props.openApp} size={{ width: this.state.width, height: this.state.height }} />)}
                    </div>
                </Resizable>
            </Draggable>
        )
...
export function WindowTopBar(props) {
    return (
        <div style={{ touchAction: 'none' }} className={"relative bg-ub-window-title border-t-2 border-white border-opacity-5 py-1.5 px-3 text-white w-full select-none rounded-b-none"}>
...
export class WindowMainScreen extends Component {
    ...
    render() {
        return (
            <div className={"w-full flex-grow z-20 max-h-full overflow-y-auto windowMainScreen" + (this.state.setDarkBg && false ? " bg-ub-drk-abrgn " : " bg-ub-grey")} data-context={this.props.dataContext}>
